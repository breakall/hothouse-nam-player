"""Validation and submodel selection for device-compatible A2-Lite captures."""
...
def validate_model(model: dict[str, Any]) -> list[float]:
    require(model.get("architecture") == "WaveNet",
            "A2-Lite submodel must be WaveNet")
    require(float(model.get("sample_rate", 0.0)) == 48000.0,
            "A2-Lite model must be 48 kHz")

    layers = model.get("config", {}).get("layers")
    require(isinstance(layers, list) and len(layers) == 1,
            "Expected one WaveNet layer stack")
    config = layers[0]
    ...
    require(config.get("kernel_sizes") == KERNEL_SIZES,
            "Unsupported A2 kernel layout")
    require(config.get("dilations") == DILATIONS,
            "Unsupported A2 dilation layout")
    ...
    require(config.get("layer1x1") == {"active": True, "groups": 1},
            "Unsupported layer1x1")
    require(config.get("head1x1", {}).get("active") is False,
            "head1x1 must be inactive")
    require(config.get("groups_input") == 1, "groups_input must be 1")
    require(config.get("groups_input_mixin") == 1,
    ...
    require(config.get("gating_mode") == ["none"] * 23,
            "Gating is unsupported")
    require(config.get("secondary_activation") == [None] * 23,
            "Secondary activations are unsupported")
    ...
    require(all(inactive_film(config.get(key)) for key in film_keys),
            "FiLM layers are unsupported")
